...
def main():
    ...
    # 计算每个样本的reward（使用generated_explanation作为输出）
    results = []
    for sample in tqdm(samples, desc="计算ground truth reward中"):
        llm_output = sample.get("generated_explanation", "")
        ref_analysis = sample.get("analysis", "")
        ground_patch = sample.get("ground_patch", "")
        buggy_lines = sample.get("buggy_location", [])

        if not llm_output:
            print("样本缺少generated_explanation字段，跳过")
            continue
