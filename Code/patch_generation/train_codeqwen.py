...
def evaluate(args, model, tokenizer, dataloader, save_path):
    ...
    for batch in tqdm(dataloader, desc="Evaluating"):
        ...
        for i in range(outputs.size(0)):
            text = tokenizer.decode(outputs[i][batch["input_ids"][i].shape[0]:], skip_special_tokens=True)
...
def dump_generation_to_json(args, model, tokenizer, dataloader, save_path, split_name="train"):
    ...
    for batch in tqdm(dataloader, desc=f"Dumping {split_name} generations"):
        ...
        for i in range(outputs.size(0)):
            gen_text = tokenizer.decode(outputs[i][batch["input_ids"][i].shape[0]:], skip_special_tokens=True)
...
def run_sft(args):
    ...
    sft_save_dir = os.path.join(args.sft_save_dir, f"codeqwen_sft_{now}")
...
def train_ppo(args):
    ...
    now = datetime.now().strftime("%Y_%m_%d_%H_%M")
    log_path = f"/home/wyx/KitPatch-63E8/Results/logs/codeqwen/log_{now}"
    ...
    for epoch in range(args.ppo_epoch):
        for step, batch in enumerate(tqdm(train_loader)):
            ...
            for i in range(response.size(0)):
                text = tokenizer.decode(response[i][input_ids.shape[1]:], skip_special_tokens=True)
    ...
    now = datetime.now().strftime("%Y_%m_%d_%H_%M")
    save_dir = os.path.join(args.save_dir, f"codeqwen_ppo_{now}")
    ...
    final_dump_dir = os.path.join(args.save_dir, "final_generations")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_model_path", type=str,
                        default="/media/models/CodeQwen1.5-7B", help="Original model path")

    parser.add_argument("--sft_save_dir", type=str,
                        default="/media/wyx/saved_models/codeqwen_sft/codeqwen_sft_2026_01_13_16_28", help="Saved model after sft")

    parser.add_argument("--save_dir", type=str,
                        default="/media/wyx/saved_models/codeqwen_ppo", help="Saved model after ppo")
