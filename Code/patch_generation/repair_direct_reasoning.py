...
def request_engine(messages):
    ...
    while ret is None:
        try:
            ret = client.chat.completions.create(model="meta-llama/llama-3.1-405b-instruct", messages=messages,
                                                 stream=False)  # gpt-3.5-turbo-1106  gpt-3.5-turbo-0125
...
def generate_patch(dataset, inference_files, output_filename="deepseek_v3_base"):
...
def main():
    ...
    with open(f"./Results/cot/bigvul/think_dataset.json", 'r') as f:
        think_dataset = json.load(f)
    with open(f"./Results/cot/bigvul/inference_dataset.json", 'r') as f:
